...
module RailsWechat::Application
  ...
  def require_login(return_to: nil)
    ...
    if current_wechat_user && current_wechat_user.user.nil?
      ...
      redirect_url = ActionDispatch::Http::URL.url_for path: '/auth/wechat', host: request.host, protocol: request.protocol, port: request.port, subdomain: false, tld_length: 1
    end
